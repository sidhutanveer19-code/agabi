 ...
 */
export async function extractStatements(chunkText: string, entityNames: string[], invoke: JsonInvoke): Promise<Advice<unknown>> {
  const { system, user } = statementsPrompt(chunkText, entityNames);
